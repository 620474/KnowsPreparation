...
export const CodePlayground = forwardRef<CodePlaygroundHandle, CodePlaygroundProps>(
  function CodePlayground({ code, runner, attemptTarget }, ref) {
    ...
    useImperativeHandle(ref, () => ({ run: () => void handleRun() }));
      ...
      <div className="code-playground">
        <div className="code-playground-heading">
          <div>
            ...
            <small>
              Код выполняется изолированно в Web Worker и останавливается через 3 секунды.
            </small>
          </div>
          <Button
            ...
            {result.tests.map((test) => (
              <div className={test.passed ? "passed" : "failed"} key={test.title}>
                ...
                <div>
                  ...
                  <small>Получено: {test.actual}</small>
                  {test.passed ? null : <small>Ожидалось: {test.expected}</small>}
                </div>
              </div>
            ))}
            {result.logs.length ? (
              <pre className="code-console">{result.logs.join("\n")}</pre>
            ) : null}
          </div>
        ) : null}
        {attemptTarget ? (
