...
- 100 вопросов для самопроверки;
- 224 проверенных источника в серверном JSON-каталоге;
...
Android получает `curriculum`, `resources` и `questions` через
`GET /api/learning/bootstrap`, обновляет запрос раз в минуту и при возвращении
в приложение. Поэтому изменения контента не требуют пересборки APK. Новая версия
APK нужна только при изменении React-интерфейса или клиентской логики.
